- [Русский](https://docs.codescoring.ru/changelog/on-premise-changelog/index.md)
...
- SCA Fixed a behavior where Johnny considered policies that triggered delayed alerts as blocking
- Fixed a behavior where ignored alerts sent email notifications and created issues in Jira
- OSA Fixed an error scanning some Maven packages
...
- SCA Added support for CycloneDX versions `1.7` and `1.7 ext`
- SCA Added a vulnerability reachability visualization page
- SCA OSA Added the `GOST:provided_by` field to the dependency edit table and to SBOM
- SCA Added the ability to specify a branch or tag when importing an SBOM
- SCA Added validation of the uploaded SBOM file format and version
- SCA Added an API endpoint with a list of supported SBOM specifications and versions
- SCA Added highlighting of vulnerable dependencies in the dependency graph when exporting a project PDF report
- SCA Added “Affected packages” and “Affected images” lists on the vulnerability page
- SCA Added the “Requirement” column to the table in SCA → Dependencies
- TQI Added filters by start date and last update date to the project list in the TQI section
- TQI Added code density and code churn metrics to charts on the TQI project page
- TQI Added display of project duration in months, lines of code count, and file count on the TQI project page
- TQI Added merge commit icons, commit type filtering, and the ability to collapse commit messages in the commits table on the TQI project page
- TQI Added scheduling configuration for author and clone analyses for a project
- TQI Added removal of outdated commits when running author analysis from the first commit
- OSA Added support for repository managers of the “Sfera.Distributions and Licenses” type
- OSA Added the `Virtual` repository type for JFrog and Nexus repositories
- OSA Added additional fields for container images (request time, policies, blocking status, registry, link)
- OSA Added the ability to specify Docker repositories for policies with OSA components and to pass the repository name when requesting an image
- OSA Added uniqueness validation when creating repository managers
- OSA Added recogninition of `cargo`, `composer`, and `huggingface` ecosystem formats in Sonatype Nexus Repository requests
- OSA Added recognition of `ai editor extensions`, `ansible`, `bazel modules`, `helm oci`, `huggingface`, `jetbrains plugins`, `nim model`, and `oci` ecosystem formats in JFrog Artifactory requests
- OSA Added PURL display on the container image page
- OSA Added filtering by OSA components in the policy list
- OSA Added the ability to pass host headers when generating a link to a blocked component in OSA
- OSA Added support for loading image data from JFrog Artifactory registries via the Repository Path API
- OSA Added updating of the last request date and scan status when scanning an image or package via the UI
- Secrets Added saving of secret coordinates
- Secrets Added secret coordinates to secret links
- Secrets Added a field for configuring the default secrets engine
- Data Added left boundary for ranges of vulnerable versions from the FSTEC Database by major version, which eliminates range overlap and reduces False Positives
- Data Added collection of metrics (CVSS v3 Rating and Threat Level) for vulnerabilities with Astra Linux
- Data Added fixed-version collection for components from the 'Possible Mitigation Measures' section of the FSTEC BDUs
...
- SCA Redesigned the vulnerability page interface
- SCA Changed the display of alert counts and dependency and vulnerability information on the project page when SCA analysis has not yet been performed
- SCA Changed the PURL comparison logic used when deciding whether to ignore an alert. See Policy ignores for details
- SCA SBOM export from the UI is now performed asynchronously (similar to CSV and PDF reports)
- SCA Updated CVSS rating filters: allowed values are now limited to the [0,10] range
- SCA Removed the has_exploit field from SBOM when no exploit is available
- TQI Redesigned the UI on TQI project and author pages
- Updated the loading indicator in the platform UI
- Changed user access with the User role to alert-related actions
- API Converted string values `True` / `False` to boolean `true` / `false` in API filter responses
...
- SCA Optimized the SBOM generation process
- SCA OSA Optimized SCA and OSA performance
- SCA Optimized vulnerability export to CSV
- SCA Fixed duplicate alerts when filtering by dependency relation or dependency environment
- SCA Fixed the `Has VCS` specified filter on the project dependency edit page
- SCA Fixed incorrect export of the match type field in the dependency report
- SCA Fixed vulnerability count calculation on the packages page
- SCA Fixed severity display when a CVSS score is present
- SCA Reduced memory consumption during intensive SCA usage
- SCA Removed alerts with delayed blocking when scanning via Johnny with the --save-results flag if the blocking time has not yet been reached
- SCA OSA Optimized API operation and UI page loading for all OSA section pages, alert list page, and SCA project groups
- TQI Optimized loading of the “Similar authors” tab on the author page
- TQI Optimized chart loading on the TQI project page
- TQI Fixed possible duplicate commits in TQI projects
- TQI Fixed the change rate calculation formula for the chart on the TQI project page
- TQI Fixed technology share calculation in project groups
- OSA Fixed display of packages from deleted repository managers in the dashboard
- Secrets Fixed saving of identical secrets with different coordinates in a file
- Secrets Fixed audit log output and logging of missing model errors when starting secrets analysis
- Data Fixed incorrect VCS links with `/sponsor` and github.io
- Data Fixed display of licenses by package versions - eliminated cases where licenses belonged to a different version
- Data Fixed incorrect FSTEC BDU versions with "X" symbol and commas
- Data Fixed incorrect VCS links in npm with /issues at the end
...
- OSA API endpoints for container image scan history have been marked as deprecated

#### Removed

- OSA Removed the container image scan history page
- TQI Removed time selection from start date and last activity date filters in the author and organization project lists
- TQI Removed the “Change rate” metrics from TQI project and author pages
...
- OSA Added `INDEX_API_TIMEOUT` and `LOG_LEVEL` settings to the Index Proxy service

#### Fixed

- OSA Fixed policy behavior in OSA Proxy without specifying `repository_id`
- SCA Fixed versioning of npm packages in SBOM files

### [2025.45.6] - 2025-12-13

#### Added

- OSA Implemented policy mechanism using cache in the OSA Proxy service

#### Fixed

- SCA Fixed SBOM file generation for projects with protestware

### [2025.45.5] - 2025-12-05

#### Added

- OSA Added automatic authorization type selection for container image registries

#### Fixed

- SCA Optimized the query for getting a list of projects on the dependency page

### [2025.45.4] - 2025-12-02

#### Fixed

- SCA Optimized PDF report export
- SCA Optimized dependency graph generation

### [2025.45.3] - 2025-11-24

#### Fixed

- OSA Fixed package display when the repository manager has been deleted
- OSA Optimized loading of the packages list
- Secrets Updated the ML-based secret detection strategy in the Secrets module

### [2025.45.2] - 2025-11-18

#### Added

- SCA Added saving of metadata when rescanning a project from the scan history

#### Fixed

- OSA Optimized package scanning performance in the OSA module
- Fixed an issue with ignore handling during OSA Proxy policy evaluation

#### Changed

- OSA Updated logging detail level in the OSA module
...
- OSA Fixed incorrect display of package blocking status

### [2025.45.0] - 2025-11-07

#### Added

- SCA Added new section `SCA → Project Groups` with aggregated metrics and bulk analysis
- SCA Added CSV export of aggregated SCA project data within a group
- SCA OSA Added component analysis for the RedOS ecosystem
- Added new filtering options for the dependency graph: filtering by parents/children and by technology
- TQI Added display of merge commits count and changed lines in project and author tables, on project/author pages and in charts
- TQI Added option to run author analysis from the first commit
- TQI Added metrics “Code Rate”, “Newness Impact” and “Churn Impact”
- TQI Added “Code Rate” chart on the project page
...
- SCA Added API set for aggregated SCA project data (see `/api/swagger` for details)
- OSA Added column and filter “Request initiator” in `OSA → Requests`
- Added sample queries with real filters for `/bulk_actions/` endpoints in Swagger
- SCA Added “Requirement” column in dependency and scan lists
- SCA Added “Requirement” field to SCA PDF report
- SCA Added “Groups” column to project list and CSV export
- SCA Added “Max fixed version” field on dependency page
- Added filters “Authors’ emails” and “Merged with” on the author merge rules page
- Secrets Added “Status” filter on project page
- Secrets Added “Status” filter on secrets list page
- Added policy ignore section to PDF report
- SCA Added “Parents” filter on dependencies page
...
- SCA Added filters on dependency vulnerabilities page
...
- OSA Added links to policies on the blocked component page
...
- OSA Removed initial relevance filters from OSA section lists
- Removed password change option for LDAP/OIDC-based users
- Secrets Removed syntax highlighting in Secrets section
...
- TQI Improved tooltip data display on project metric charts
...
- SCA Moved `sca-tasks` queue to celery worker
- SCA Removed unicode encoding of text in SBOM files
...
- OSA Fixed deadlock when scanning multiple packets with the same name but different qualifiers
- SCA Improved SCA analysis performance
- SCA Improved SBOM generation performance during SCA analysis
- Fixed reset of “Source files” field after rescan
- Fixed policy actions edit behavior
- Secrets Fixed secrets list display
...
- OSA Optimized loading of images from registries
- SCA Optimized the API request for getting project dependencies
...
- SCA Fixed an error determining paths to the `.csproj` and `project.assets.json` manifests when scanning .NET projects
...
- SCA Added a database migration to correct the archived flag for alerts generated by SCA analyses

### [2025.37.1] - 2025-09-25

#### Added

- OSA Added ability to deactivate the repository manager
...
- OSA Fixed issue with ignoring alerts for OSA packages
...
- SCA Fixed issue in the "PURL contains" policy: if the checked element was not a valid PURL, the policy did not trigger
- SCA Fixed issue when SCA scanning projects with dependencies having identical name, version, and language
- Fixed issue with enabling inactive policies
- Secrets Fixed error when exporting secrets PDF report
...
- SCA Added vulnerability reachability analysis for Java, powered by the Svace call graph builder
- SCA Added policy "Vulnerability is reachable"
- SCA Added UI display of the reachability attribute for vulnerabilities
- SCA Added ability to specify `component manufacturer` data to be placed in the relevant section of SBOM versions `CycloneDX 1.6` and `CycloneDX 1.6 ext`, both at platform level and per project
...
- SCA Added saving of annotated data when importing SBOM
- SCA Added support for SBOM export in SPDX format
- SCA Added suspicious commit links for vulnerabilities with CSPW identifiers in SBOM
- Added integration with external identity providers implementing the OpenID Connect protocol
- SCA Added localization of PDF reports in the SCA module
- Added saving of links for Jira issues and emails created by automatic actions with alerts
- SCA Added "Requirement" field displaying the required version range from the manifest in the project dependencies section
...
- SCA Added "Release Date" field to the project dependencies list
...
- SCA Added project search and filters by project, relation, detection type, and environment for individual vulnerabilities on the dependencies page
- TQI Added ability to calculate the number of unique authors in GitLab outside of analysis runs
- SCA Added ability to run SBOM scans from scan history to check for new vulnerabilities in historical component data
- SCA Added "Max fixed version" field to the dependencies section of the project PDF report
- TQI Added filtering by multiple authors on the "List" and "Activity Map" tabs in the `TQI -> Authors` section
- TQI Added tooltip popup with author’s projects when hovering over author’s project count in the `TQI -> Authors` section
- TQI Added author filter on the project list page in the TQI section
- TQI Added "Number of Authors" chart on the project page in the TQI section
- TQI Added "Author’s Commits" and "Author’s Projects" charts on the author page in the TQI section
...
- Secrets Added ability to export a CSV report with secrets on the project tab
- Secrets Added ability to generate a PDF report with secrets on the project tab

#### Changed

- SCA Improved parsing of VCS links in CycloneDX SBOM
- TQI Improved the mechanism for merging authors by email
- Unified the UI for running alert actions manually and automatically
- SCA Commit hash in the project PDF report is now displayed in full
...
- OSA Changed detailed package info display on the OSA module package page
- TQI Fixed display of "Change author mapping" button when TQI module is disabled
- Changed `GOST:source_lang` field to `GOST:source_langs` in all exported files
- TQI Added information on the number of months an author committed to the TQI section
- TQI Project parameter "Duration (in months)" now shows integer number of months of project activity
...
- SCA Optimized loading of dependency and vulnerability lists
- Secrets Changed display and content of tooltips about inability to train ML model in Secrets module
- Changed "Deferred blocking" filter type on the policies page

#### Fixed

- SCA Fixed alert table format in project PDF report
...
- SCA Fixed link handling in CycloneDX SBOM
- SCA Implemented missing logic for handling alerts when comparing PURL values with case sensitivity
- Fixed auto-generated type names in OpenAPI schema
- Fixed verb forms of policy conditions in English locale
- TQI Fixed project links in "Organization Projects" tab on the author page
- TQI Fixed links to code duplicates in the duplicates map
- TQI Fixed CSV export of author’s projects
...
- SCA Fixed missing options in dependency environment filter when `USE_SMART_FILTERS` is set to `False`
- SCA Fixed error when scanning a project with dependencies containing invalid PURLs
- Fixed search in dropdowns for Russian locale
- TQI Fixed "Refactoring candidate projects" and "Intra-project duplicates" widgets on the dashboard
- Fixed search within condition dropdowns on the policy editing page
- SCA Fixed behavior of "Dependency is dangerous" policy in SCA module
- SCA Fixed policy execution error with condition `PURL exactly_match`
...
- SCA Optimized loading of dependencies page in SCA module
...
- SCA Added the ability to view the results of SCA analysis of a project from the [scan history](/sca/scan-history.en)
...
- SCA Added the ability to download the current version of the Johnny binary agent directly from the platform
...
- SCA Added legend for the project dependency graph
- SCA Added analysis identifier to webhooks associated with the completion of SCA analysis
- OSA Added output of the archive/activity flag for OSA packages, container images and alerts
...
- OSA The OSA background package update mechanism has been optimized. Only relevant packages are updated. By default, packages requested in the last 14 days are considered relevant, the parameter is configured in the platform settings
...
- SCA Updated Johnny version on platform to 2025.29.1
...
- SCA Fixed image hash validation error when passing value via `--hash` parameter when scanning image via Johnny
- Fixed some API schema inaccuracies in Swagger
- OSA Fixed error processing list of entities from Docker Registry when receiving `null` instead of empty list
- Fixed error processing image signature files with `.sig` extension and `.att` when working with container registries
- TQI Fixed an error on the "Similar Authors" tab on the author profile page in the case of authors without certain technologies
...
- SCA Fixed the reset of the dependency graph state when switching to another browser tab
- Fixed problems updating data on the page after editing some entities
- OSA Fixed deadlocks when using multiple instances of the OSA Registration service
...
- OSA Fixed the output of the list of authors on the page with detailed information about the package blocking in the OSA module
