- [English](https://docs.codescoring.ru/changelog/nexus-changelog.en/index.md)
...
- Добавлена обработка неблокирующего статуса `delayed_block`
- Добавлена настройка `Append block URL to custom message` для управления добавлением ссылки на блокировку в кастомное сообщение
- Добавлена поддержка conda-репозиториев

#### Исправлено

- Исправлена проблема сохранения Capability `Configuration`: если поля `Pool Size` и `Timeout` пустые, после снятия флага `Block downloads in case of plugin...` изменения теперь корректно сохраняются
- Исправлена проблема со сканированием maven пакетов, содержащих версию в artifactId
...
- Улучшено [логирование ответов от платформы](/osa/nexus_osa/#nexus-logging)
