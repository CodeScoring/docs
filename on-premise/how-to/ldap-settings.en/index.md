- [Русский](https://docs.codescoring.ru/on-premise/how-to/ldap-settings/index.md)
...
On the authentication page, a menu is available with a choice of authentication provider. In addition to the default provider (local accounts, `internal directory`), active integrations with **LDAP** servers are available for selection.
...
Viewing existing integrations is available in the `Settings -> LDAP` section. The section displays a table with a list of configured LDAP integrations, a button to create a new integration (`Setup new`), and a search box (`Search`).
