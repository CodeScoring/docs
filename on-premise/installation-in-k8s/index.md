- [English](https://docs.codescoring.ru/on-premise/installation-in-k8s.en/index.md)
...
1. Создать secret для доступа к приватному реестру Docker-образов системы "CodeScoring", используя адрес (`REGISTRY_URL`), логин (`USERNAME`) и пароль (`PASSWORD`), полученные от вендора.
...
1. Установить [Helm](https://helm.sh/docs/intro/install/) предпочтительным способом.

1. Выполнить следующие команды для добавления актуального Helm-репозитория на локальную машину:
...
1. Создать файл `values.yaml` со следующим содержимым:
...
1. Выполнить команду для установки чарта
...
1. В переменных `codescoring.config.djangoCachesRedisUrls` и `codescoring.config.hueyRedisUrl` указать строки подключения для внешнего Redis.
...
1. Добавить корневой сертификат сервера Redis в `codescoring.trustedCA.certificates`
1. В переменных `codescoring.config.djangoCachesRedisUrls` и `codescoring.config.hueyRedisUrl` указать строки подключения для внешнего Redis в формате `rediss://redis.example.com:6379/0`, где 0 - номер базы данных в Redis.
...
1. Подключить пулер Pgbouncer к внешней PostgreSQL, заменив соответствующие параметры на нужные:
...
1. Отключить развертывание PostgreSQL, указав переменную - `postgresql.enabled: false`
1. Отключить развертывание Pgbouncer, указав переменную - `pgbouncer.enabled: false`
1. Подключить codescoring напрямую к внешнему пулеру, в секции `codescoring.config` параметры:
...
1. Задать путь до **каталога на ноде кластера**, в котором будут размещены данные в следующих переменных:
...
1. Указать название ноды, на которой будет создан локальный том в следующих переменных:
...
В результате будут созданы PersistentVolumeClaim для соответсвующих сервисов. Стоит отметить, что возможности конфигурирования данных томов полностью соответствуют описанным в секции [Настройка томов (PV)](#volumes).
...
1. Добавить корневой сертификат удостоверяющего центра (RootCA) в формате PEM в переменную `codescoring.trustedCA.certificates` в формате `ключ: значение`, где ключ - имя файла сертификата, включая расширение `.crt`, значение - сертификат в формате PEM.
