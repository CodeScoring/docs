- [Русский](https://docs.codescoring.ru/osa-proxy/base64-url/index.md)
...
1. Select the desired type (e.g., `maven2 (proxy)`).
1. In the **Remote storage** field, enter the URL of your `osa-proxy` instance, including the repository name and Base64-encoded parameters.
...
1. In the configuration, set the **URL** field to the `osa-proxy` URL. This URL must include the repository name and the Base64-encoded string.
...
1. **Encode the string using URL-safe Base64.**
