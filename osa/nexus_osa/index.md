- [English](https://docs.codescoring.ru/osa/nexus_osa.en/index.md)
...
1. Выдать права для пользователя и группы `nexus`:
...
1. Проверить, что у пользователя есть минимальный набор привилегий для корректной работы с плагином:
...
1. **Использование команды `java` с параметром `-XshowSettings:properties`**:

   ```
   java -XshowSettings:properties 2>&1 > /dev/null | grep 'java.home'
   ```

1. **Использование команды `readlink` с путем к `java`**:
