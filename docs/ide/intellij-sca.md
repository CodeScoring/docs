...
# Плагин CodeScoring SCA для IntelliJ IDEA

Плагин предоставляет возможности анализа состава программного обеспечения (SCA) для IntelliJ IDEA и других IDE от JetBrains, подсвечивая уязвимые зависимости в файлах вашего проекта и предоставляя подробную информацию об уязвимостях через интеграцию с Johnny CLI.
...
2. Перейдите в **File** → **Settings** (или **IntelliJ IDEA** → **Preferences** на macOS)
...
10. Перезапустите IntelliJ IDEA при появлении запроса
...
1. Откройте веб-браузер и перейдите по адресу: `<API URL>/cabinet/profile`
    - Например: `https://staging.demo.codescoring.com/cabinet/profile`
...
7. Вернитесь к настройкам IntelliJ IDEA
...
Теперь, когда плагин настроен, вы можете запустить первое сканирование уязвимых зависимостей:
...
1. **config.yaml** - файл конфигурации для Johnny CLI, вы можете прочитать о нем [здесь](./../agent/config.md). Вы можете менять этот файл, так как он никогда не будет перезаписан.
...
3. **bom.json** - файл результатов сканирования, созданный Johnny CLI в формате cyclone-dx 1.6, он будет загружен автоматически и показан в панели уязвимостей для любого открытого в IntelliJ проекта, в котором существует .codescoring/bom.json
4. **bom.json.N** - где N - это ревизия сканирования, т.е. 0 - предыдущее сканирование, а 5 (например) - самое первое сканирование, и bom.json.0 будет использоваться для сравнения с bom.json (и показан в дереве DIFF), если он существует на момент открытия проекта
...
#### 6.1 Пример использования конфигурации сканирования CodeScoring
...
1. **Проверьте уведомления**: Посмотрите на уведомления в правом нижнем углу IntelliJ IDEA, нажав на **"View Report"** и **"See details in Vulnerabilities view""**
...
#### 7.1 Дополнительные функции

##### 7.1.1 Подсветка уязвимостей
...
##### 7.1.2 Информация при наведении
...
##### 7.1.3 Панель уязвимостей

###### 7.1.3.1 Структура деерва
```
📊 Уязвимости (247)
├── 🔴 Критические (12)
...
- **Update vulnerable dependnecy** внизу на всплывающей карточке уязвимого компонента
...
| Проблема | Решение                                                                                |
|----------|----------------------------------------------------------------------------------------|
| Плагин не виден после установки | Полностью перезапустите IntelliJ IDEA, проверьте **Settings** → **Plugins**            |
...
Свяжитесь с отделом заботы: `mailto:hello@codescoring.ru`
