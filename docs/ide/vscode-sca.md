...
Расширение поставляется в виде файла платформенно независимого файла `codescoring-sca-<version>.vsix`.

### Шаг 2: Установите расширение из файла VSIX

1. Откройте Visual Studio Code
2. Перейдите в **Файл** → **Настройки** → **Расширения** (или нажмите `Ctrl+Shift+X` или `Cmd+Shift+X` на Mac)
...
1. Нажмите `Ctrl+Shift+P` (или `Cmd+Shift+P` на Mac), чтобы открыть палитру команд
...
![Снимок экрана панели уязвимостей, показывающей обнаруженные проблемы с уровнями серьезности](./../assets/img/ide/vscode/step6-2-vulnerabilities.png)
...
###### 7.1.3.1 Структура древовидного представления
```
📊 Уязвимости (247)
├── 🔴 Критические (12)
...
- **По серьезности -> Расположению** (по умолчанию): По уровню серьезности, подгруппировка по пакету
- **По серьезности -> Пакету (PURL)**: По уровню серьезности, подгруппировка по имени пакета в алфавитном порядке
...
- При нажатии `Ctrl+.` (`Cmd+.` на Маке), когда курсор находится на уязвимом компоненте, Вам предложится выбрать конкретную версию для обновления (при наличии нескольких)
...
- **Кнопка Fix All**: Обновляет все уязвимые компоненты с доступными исправлениями, видимые в данный момент в дереве, при этом учитывает примененные фильтры (лупа)
...
###### Ручные операции
- **Загрузить BOM**: `Ctrl+Shift+P` (или `Cmd+Shift+P` на Mac) → "Load BOM File"
...
- **Открывается в**: Предпросмотре HTML в VS Code
- Адаптируется к выбранной цветовой схеме VSCode в момент генерации

##### 7.1.8 Настройки и кастомизация
...
| Экосистема | Файлы манифеста | Файлы блокировки |
|------------|-----------------|------------------|
