...
1. config.yaml - файл конфигурации для Johnny CLI, вы можете прочитать о нем [здесь](./../agent/config.md). Вы можете
...
- При нажатии `Ctrl+.` (`Cmd+.` на Маке), когда курсор находится на уязвимом компоненте, Вам предложится выбрать
...
- **Кнопка Fix All**: Обновляет все уязвимые компоненты с доступными исправлениями, видимые в данный момент в дереве,
  при этом учитывает примененные фильтры (лупа)
