...
- Добавлен анализ достижимости уязвимостей с поддержкой Svace и Java
- Добавлена политика "Уязвимость достижима"
- Добавлена интеграция с внешними провайдерами идентичности, реализующими протокол OpenID Connect
- Добавлена локализация PDF-отчетов
- Добавлены мета-данные созданных артефактов в рамках автоматических действий по политикам
- Добавлено поле "Требование", отображающее диапазон требуемых версий в манифесте, в раздел зависимостей проекта
- Добавлено необязательное поле "Приоритет" в форму действия по созданию задач в Jira
- Добавлена возможность создавать кастомные шаблоны для дейcтвий с политиками, по умолчанию используется встроенный шаблон
- Добавлен скрипт перешифровки чувствительных данных при смене токена `SECRET_KEY`
- Добавлена возможность указать данные `component manufacturer`, которые будут размещены в соответствующем разделе SBOM версий `CycloneDX 1.6` и `CycloneDX 1.6 extНа`, на уровне инсталляции и для каждого проекта в отдельности 
...
- Добавлено поле "Дата выпуска" в список зависимостей проекта
...
- Добавлены ссылки на подозрительные коммиты для уязвимостей с идентификаторами типа CSPW в SBoM
- Добавлен поиск по проектам и фильтр по проекту, связи, типу обнаружения и окружению для отдельной уязвимости
- Добавлена возможность сканировать VCS для расчёта количества уникальных авторов
- Добавлена возможность запуска сканирования SBoM в истории сканов с целью проверки новых уязвимостей в исторических данных по компонентам
- В раздел зависимостей PDF отчета проекта добавлено поле "Максимальная исправленная версия"
- Добавлена возможность фильтрации по нескольким авторам на вкладках "Список" и "Карта активности" в разделе `TQI -> Авторы`
- Добавлено всплывающее окно со списком проектов автора при наведении на количество проектов автора на странице `TQI -> Авторы`
- Добавлен фильтр по авторам на странице проектов в разделе TQI
- Добавлен график "Количество авторов" на странице проекта в разделе TQI
- Добавлены графики "Коммиты автора", "Проекты автора" на странице автора в разделе TQI
- Добавлена возможность изменять названия проектов
- Добавлено сохранение размеченных данных при импорте SBoM
...
- Добавлена возможность экспорта CSV-отчета с секретами на вкладке проекта
- Добавлена возможность генерации PDF отчета с секретами на вкладке проекта
- Добавлено отображение в UI признака достижимости для уязвимостей

#### Изменено

- Приведен к единому виду интерфейс запуска действий с политиками
- Хэш коммита в PDF-отчете проекта теперь выводится полностью
...
- Изменен вывод детальной информации на странице пакета в модуле OSA
- Исправлено отображение кнопки "Изменить сопоставление авторов" при отключенном модуле TQI
- Изменено поле `GOST:source_lang` на `GOST:source_langs` во всех экспортируемых файлах
- В раздел TQI добавлена информация о количестве месяцев, когда автор делал коммиты
- Параметр проекта "Продолжительность (в месяцах)" теперь отображает целое количество месяцев активности проекта 
- Удалено поле `internal` из ответа метода API `/api/activation_keys/`
- Разделено API проектов по модулям (SCA, TQI, Secrets)
- Базовый образ Index Proxy переведен на Alpine Linux
- Улучшена производительность истории сканирований через API
- Оптимизирована загрузка страницы зависимостей и уязвимостей
- Изменено отображение и содержание текста подсказок о невозможности обучения ML-модели в модуле Secrets
- Изменен тип фильтра "Задержка блокировки" на странице политик

#### Исправлено

- Исправлен формат таблицы c алертами в PDF-отчете проекта
...
- Исправлена обработка ссылок в CycloneDX SBoM
- Реализована недостающая логика для работы алертов при сравнении значения PURL с учетом регистра
...
- Исправлены ссылки на проекты на вкладке "Проекты организации" на странице автора
- Исправлены ссылки на дубликаты кода в карте дубликатов
- Исправлена выгрузка проектов автора в CSV
...
- Исправлено отсутствие опций в фильтре по окружению зависимостей если параметр `USE_SMART_FILTERS` имеет значение `False` 
- Исправлена ошибка при сканировании проекта, в зависимостях которого есть пакеты с невалидным PURL
- Исправлен поиск в выпадающих списках в русскоязычной локали
- Исправлены виджеты "Проекты-кандидаты на рефакторинг" и "Внутрипроектные дубликаты" на панели мониторинга
- Исправлен поиск внутри выпадающих списков условий на странице редактирования политики
- Исправлена работа политики "Зависимость опасна" в модуле SCA
- Исправлена ошибка работы политики с условием `PURL exactly_match`
...
- Оптимизирована загрузка страницы зависимостей в модуле SCA
...
- Добавлена возможность посмотреть результаты SCA-анализа проекта из истории сканов
- Добавлена возможность кастомизации экспорта SBoM и PDF файлов
- Добавлена возможность ручного создания задания в менеджере задач и отправки писем для выбранных алертов
...
- Добавлено условие для политики `Зависимость является потомком` для поиска дочерних зависимостей нужного пакета на любом уровне графа зависимостей
- Добавлена колонка `Максимальная исправленная версия` в таблице зависимостей
- Добавлена возможность скачать актуальную версию консольного агента Johnny напрямую из инсталляции
...
- Добавлена легенда для графа зависимостей проекта
- Добавлен идентификатор анализа в вебхуки, связанные с завершением SCA-анализа
- Добавлен вывод признака архивности/активности для OSA-пакетов, контейнерных образов и алертов
...
- Оптимизирован механизм фонового обновления OSA-пакетов пакетов. Обновляются только актуальные пакеты. По умолчанию, актуальными считаются пакеты, запрошенные за последние 14 дней, параметр конфигурируется в настройках инсталляции
...
- Улучшена логика отображения раздела управления ML-моделью в модуле Secrets
...
- Изменён базовый образ в сервисе OSA API с Debian bookworm на Alpine
...
- Обновлена версия gitleaks для модуля Secrets до 8.27.0
- Обновлен образ Redis с 7.0.12 до 7.4.3
- Обновлен образ PostgreSQL с 13.4 до 13.21
- Обновлена версия Johnny на инсталляции до 2025.29.1
...
- Исправлена ошибка валидации хэша образа при передаче значения через параметр `--hash` при сканировании образа через Johnny
- Исправлены некоторые неточности схемы API в Swagger
- Исправлена ошибка обработки списка сущностей из Docker Registry при получении `null` вместо пустого списка
- Исправлена ошибка обработки файлов подписи образов с расширением `.sig` и `.att` при работе с реестрами контейнеров
- Исправлена ошибка на вкладке "Схожие Авторы" на странице профиля автора в случае наличия авторов без определенных технологий
...
- Исправлен сброс состояния графа зависимостей при переходе на другую вкладку браузера
- Исправлены проблемы обновления данных на странице после редактирования некоторых сущностей
- Исправлены взаимные блокировки при использовании нескольких экземпляров сервиса OSA Registration
...
- Исправлен вывод списка авторов на странице с детальной информацией о блокировке пакета в модуле OSA
...
- Добавлена маскировка чувствительных данных в логах инсталляции
