...
- Добавлена настройка `USE_SMART_FILTERS` (по умолчанию значение **True**). Если установить её в **False**, фильтры будут показывать все возможные значения. Если оставить **True**, фильтры будут показывать только те значения, которые актуальны для уже отфильтрованных данных
