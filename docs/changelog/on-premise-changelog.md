...
- <span class="module-tag sca">SCA</span> Добавлена возможность указать данные `component manufacturer`, которые будут размещены в соответствующем разделе SBOM версий `CycloneDX 1.6` и `CycloneDX 1.6 ext`, на уровне инсталляции и для каждого проекта в отдельности 
...
- <span class="module-tag tqi">TQI</span> Улучшен механизм объединения авторов по почтам. Теперь алгоритм выдает на 35% меньше ложных результатов
