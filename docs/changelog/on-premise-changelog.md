...
- Добавлена локализация интерфейса на русский язык в бета-версии, переключение языков доступно на странице профиля пользователя 
...
- Добавлена выгрузка в формат CycloneDX 1.6 Ext с добавлением полей `GOST:source_lang`, `GOST:attack_surface` и `GOST:security_function` для соответствия ГОСТу по защите информации и композиционному анализу ПО. Поля заполняются значением по умолчанию.
...
- Исправлена загрузка списка контейнерных образов из реестров в случае, если из некоторых образов не удалось получить метаданные
