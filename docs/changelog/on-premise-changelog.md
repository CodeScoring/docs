...
- Устранено увеличение времени сканирования образов
- Изменен подход к перерасчету политик образов при обновлении информации об уязвимостях пакета, чтобы политики для одних и тех же образов не перерасчитывались несколько раз
- Для повторного сканирования пакетов теперь используется отдельная очередь (celery), чтобы не нагружать очередь сканирования пакетов в OSA
- Добавлены команды:
  - Для получения списка huey очередей: `./manage.py runscript get_huey_queues`
  - Для сброса huey очереди: `./manage.py runscript reset_huey_queue --script-args queue1 queue2`
  - Для сброса ключа с датой последнего обновления уязвимостей пакетов: `./manage.py runscript delete_vulnerable_dependency_updater_cache_key`
  - Для внеочередного запуска обновления уязвимостей пакетов: `./manage.py runscript run_update_vulnerable_dependencies`
