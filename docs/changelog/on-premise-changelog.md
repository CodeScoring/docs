...
- <span class="module-tag sca">SCA</span> <span class="module-tag osa">OSA</span> Добавлена поддержка экосистемы RedOS
...
- <span class="module-tag sca">SCA</span> Добавлена очередь `sca-tasks` в celery worker
- <span class="module-tag sca">SCA</span> Добавлен набор API для отображения данных проектов в разрезе SCA, агрегированных по группам, подробнее в `/api/swagger`
    - `/api/sca/groups/dashboard/` — агрегированные данные для всех групп
    - `/api/sca/groups/async_csv/` — выгрузка данных в CSV с поддержкой выбранного языка пользователя
    - `/api/sca/groups/{id}/` — агрегированные данные для конкретной группы, включая распределение по технологиям
    - `/api/sca/groups/{id}/by_components/` — статистика зависимостей и уязвимостей для группы
    - `/api/sca/groups/{id}/by_licenses/` — статистика лицензий
    - `/api/settings/groups/{id}/bulk_add_projects/` — массовое добавление проектов в группу
    - `/api/settings/groups/{id}/bulk_remove_projects/` — массовое удаление проектов из группы
    - `/api/analyses/groups/sca/start/` — запуск анализа
    - `/api/analyses/groups/sca/status/` — статус последнего запущенного анализа
    - `/api/analyses/groups/sca/cancel/` — отмена последнего запущенного анализа
- Для всех API зависимостей добавлен ключ `name_and_version`, содержащий имя и версию пакета; в UI теперь отображается это значение
...
- Добавлено поле `matched_criteria_data` в запрос API `/integration/run_analysis`
- Добавлено копирование действий существующей политики при создании новой через запрос API `/policies/`
...
- <span class="module-tag sca">SCA</span> Добавлена поддержка русского языка в файлах SBoM
- Добавлены ссылки на конкретную уязвимость CSPW в отчёте Johnny
...
- Переименовано поле "Проект" в "Проекты" с поддержкой множественного выбора на страницах списка уязвимостей и списка алертов
- Изменены форматы отображения данных в колонках "Сервер LDAP" и "Сервер OIDC" на странице списка пользователей
- Изменён возвращаемый `content-type` для набора API:
    ...
    - `/api/media/download/`
- Изменены права доступа для набора API (актуальные права можно увидеть в разделе [управления учетными записями](on-premise/how-to/users/):
...
- <span class="module-tag tqi">TQI</span> Улучшено отображение данных во всплывающем окне при наведении на графики
- Изменён контент модального окна уведомления о новой версии
- Изменена структура и наполнение бокового меню
- Упрощена работа с формой создания/редактирования провайдера OIDC
- Изменён расчёт задержки блокировки по политике — теперь по точным часам
- Изменён способ получения значений фильтров с константными значениями
- Улучшен перевод полей в системе
...
- Исправлен формат таблицы зависимостей в PDF-отчёте
- Исправлен формат таблицы уязвимостей в PDF-отчёте
- Исправлены переводы пунктов меню и терминов в интерфейсе
...
- <span class="module-tag sca">SCA</span>  <span class="module-tag osa">OSA</span> Оптимизированы анализы SCA и OSA
- Оптимизировано формирование SBOM
...
- Исправлено отображение списка секрета в <span class="module-tag secrets">Secrets</span>
