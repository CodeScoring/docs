...
- <span class="module-tag osa">OSA</span> Добавлена поддержка менеджеров репозиториев типа «ИТ-Платформа СФЕРА»
...
- <span class="module-tag secrets">Secrets</span> Добавлено поле "Entropy / Энтропия" в описание найденного секрета
- <span class="module-tag secrets">Secrets</span> Добавлено сохранение координат секретов в файле
...
- <span class="module-tag osa">OSA</span> Добавлено формирование PURL для контейнерных образов
...
- Добавлены списки значений у условий при редактировании политики
- <span class="module-tag sca">SCA</span> Добавлена колонка «Требование» в таблице раздела `SCA → Зависимости`
- Добавлена дополнительная метаинформация на странице просмотра политики
- Добавлена возможность отвязать задачу от алерта
...
- <span class="module-tag sca">SCA</span> Изменён способ сравнения PURL при принятии решения об игноре алерта
- <span class="module-tag sca">SCA</span> Экспорт SBOM из интерфейса теперь выполняется асинхронно (аналогично CSV и PDF отчётам)
- <span class="module-tag sca">SCA</span> Консольный агент Johnny исключён из процесса композиционного анализа CLI-проектов; данные о зависимостях берутся из базы данных
- <span class="module-tag sca">SCA</span> Изменены фильтры рейтинга CVSS: допустимые значения ограничены диапазоном [0,10]
- Изменён индикатор загрузки
- В ответах API строковые значения `True` / `False` приведены к булевым `true` / `false`
- <span class="module-tag sca">SCA</span> Изменена логика отображения `has_exploit` в SBOM
- Выполнен рефакторинг системы прав доступа
- Теперь при сканировании образа или пакета через UI компонент регистрируется, обновляя дату последнего запроса и статус сканирования
- Теперь при сканировании через Johnny с флагом `--save-results` не возвращаются алерты с отложенной блокировкой, если время блокировки ещё не наступило
- Для запроса UserInfo в OIDC теперь используется HTTP-метод `GET`
...
- Исправлено форматирование чисел на графиках
- Исправлена фильтрация по булевым типам
...
- <span class="module-tag osa">OSA</span> API-эндпоинты истории сканирования контейнерных образов помечены как устаревшие; планируемое удаление — релиз 2026.11.0
