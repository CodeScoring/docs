...
- Переработано создание и изменение политик: теперь указывать проект можно независимо от выбранных групп и владельцев
- Добавлена ссылка на менеджер репозиториев на странице пакета в OSA 
- Добавлен фильтр по технологиям и соответствующая колонка в раздел "Алерты" (скрыта по умолчанию)
- Добавлена возможность выбрать ветку или тег при запуске сканирования SCA для VCS проектов
...
- Добавлена колонка "Заметка" в раздел "Игноры политики" (скрыта по умолчанию)
- Добавлены фильтры "Тип", "Тип авторизации", "Активен" и поиск по названию и адресу в раздел "Реестры" 
- Добавлено ограничение количества запросов на логин от одного и того же пользователя в единицу времени 
- Добавлен часовой пояс в дате генерации PDF-отчета
- Добавлена поддержка TLS-шифрования для PostgreSQL и PgBouncer в compose
- Добавлена поддержка малых экранов для большинства разделов
- Добавлен сокращённый вывод родительских зависимостей в таблице зависимостей у проекта
- Добавлен сокращенный вывод событий в таблице вебхуков
- Добавлен вывод ошибок для формы фильтров у таблиц
- Добавлены фильтры на страницу истории сканирований SCA
- Добавлена маскировка чувствительных данных в ответах проверки подключения к VCS
- Обновлена информация metadata tools при выгрузке SBoM в CycloneDX. Новые метаданные будут видны при новых сканах
- Улучшен интерфейс на русском языке
- Изменен способ загрузки опций для фильтров таблиц: теперь список опций загружается только при необходимости
- Изменен тип пагинации на CursorPagination для фильтров
- Произведены доработки и улучшения карт активности, сложности и дубликатов
- Исправлена ошибка сортировки уязвимостей образов по Fixed Version
- Уже существующие в группе проекты и пользователи теперь не будут предлагаться для выбора 
- Теперь пользователю показываются только те данные на панели мониторинга, которые доступны в рамках его роли
- Cнижено потребление памяти при выгрузке данных о проектах в CSV
- Исправлен вывод ошибки в UI при попытке создать уже существующий проект 
- Исправлена ошибка в случае, когда кнопка "Экспорт в PDF" в истории SCA сканирований была активна при неудачном анализе и отсутствии SBoM
- Исправлен подсчет количества запущенных задач в метриках celery
- Исправлено возвращение некорректной ошибки при недоступности Index API
- Исправлена ошибка в случае, когда у CLI-проекта отсутствовало первое и последнее время сканирования
- Исправлена ошибка в случае, когда анализы, запущенные через CLI, получали завершенный статус вне зависимости от фактического исхода сканирования
- Исправлено обновление данных пользователя после сохранения формы редактирования
