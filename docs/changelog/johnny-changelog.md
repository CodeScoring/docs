...
- Добавлен флаг `--localization` для локализация результатов работы агента (форматы coloredtable, table, text, csv) 
...
- Добавлен вывод импакта уязвимости при выводе результата в csv 

#### Изменено
- Для отчёта Секретов добавлен Severity Critical для каждого секрета
- Пути при сканировании секретов изменены на относительные
- Выводится весь список обработанных агентом манифестов, даже если в них не обнаружено зависимостей
- Общая информация по результату анализа всегда выводится в консоль, даже при перенаправлении вывода в файл
