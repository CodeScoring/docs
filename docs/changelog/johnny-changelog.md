...
- Добавлена валидация при сканировании SBOM (поддерживаемые форматы: 1.4, 1.5, 1.6, 1.6_ext)
- Добавлена поддержка выгрузки SBOM в формате 1.7
- Добавлена поддержка графа вызовов Svace для языка Kotlin
...
- Для отчёта по найденным секретам добавлен уровень угрозы `Critical` для каждого секрета
- Пути при сканировании секретов изменены на относительные
- Теперь выводится весь список обработанных агентом манифестов, даже если в них не обнаружено зависимостей
- Теперь общая информация по результату анализа всегда выводится в консоль, даже при перенаправлении вывода в файл
- Изменена логика определения релиза ОС для команды `scan build`
- Библиотеки, не прошедшие проверку по ldconfig добавляются в список unresolved в команде `scan build`
