...
- Добавлен анализ достижимости уязвимостей для языка Java и графа вызовов Svace (начиная с версии 2025.37.0 инсталляции)
- Добавлена возможность запуска локального сканирования без указания параметров активации (`--key` или `--api_url` и `--api_token`) с формированием в результате SBoM
- Добавлена поддержка манифестов `deps.json` для Nuget
- При анализе SBoM командой `scan bom`, добавлена загрузка компонентов для которых PURL не указан, либо указанный PURL не соответствует спецификации
- Добавлено предварительное конвертирование файла в кодировку UTF-8 в команде `scan bom`  для дальнейшей корректной обработки
- Добавлена возможность запуск команд `sign bom` и `verify bom` без указания параметров активации (`--key` или `--api_url` и `--api_token`)
- Добавлен вывод примеров достижимости в SARIF
- Добавлена передача размеченных данных на инсталляцию при импорте SBoM с разметкой (начиная с версии 2025.37.0 инсталляции)
- Добавлена возможность указания ветки/тега и коммита при сканировании образа
- Добавлена поддержка манифестов sln для стека .NET
- Добавлена проверка состава лицензии перед выполнением сканирования
- Добавлена возможность выбора типа прогресс-бара

#### Изменено

- Изменен уровень логирования при срабатывании политики
...
- Доработан механизм связывания для gradle при использовании произвольных имен манифестов

#### Исправлено

- Исправлена обработка некорректного пакета в манифестах `requirements.txt`
