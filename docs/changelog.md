...
- Добавлена переменная `JFROG_PLUGIN_VERSION` в окружение
- Добавлен префикс `CodeScoring: ` ко всем логам

### [2023.28.1]

- Понижен уровень всех ошибок взаимодействия с инсталляцие CodeScoring до `[INFO]`

### [2023.28.0]

- Исправлена ошибка с неработающим флагом `blockDownload`

### [2023.27.0]

- Добавлен флаг `blockDownloads` в properties, который позволяет качать пакет при ошибках парсинга и не ОК ответах CS
- Исправлено выставление статуса запроса

### [2023.26.0]

- Исправлена ошибка, в которой при скачивании пакета через virtual репозитории не проставлялись properties
...
- Теперь плагин инициализируется единожды, а не на каждом запросе;

## NXRM Firewall Plugin
