...
Для корректного анализа пакетов необходимо указать название дистрибутива из удалённого репозитория. Это название вписывается в поле **Internal Description**. Оно используется в PURL (Package URL) для повышения точности анализа пакета.  
