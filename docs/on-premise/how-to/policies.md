...
- **Active** – список сработавших политик по результатам последнего анализа (проекта, образа или компонента в прокси-репозитории);
- **Ignored** – список проигнорированных политик;
- **Resolved** – список политик, которые были разрешены (условие больше не актуально).

Причина срабатывания политики отображается в поле **Matched criteria**, включая заданные условия и найденные данные о компоненте. Например, значение `CVSS3 Score >= 3.00 (9.1)` подразумевает, что политика блокирования компонентов с рейтингом CVSS3 равным или выше 3.00 сработала на компоненте с рейтингом 9.1.

## Игнорирование политик

Созданные политики можно временно или навсегда проигнорировать при анализе. Условие игнорирования позволит оставить политики в системе, при этом не получая уведомлений о ее срабатывании. Создание и настройка условий игнорирования происходит в разделе `Settings -> Policy Ignores`.
