...
Для добавления в систему проектов (git репозиториев) на анализ необходимо предварительно создать подключение к системе контроля версий. CodeScoring поддерживает следующие популярные репозитории хранения кода:
