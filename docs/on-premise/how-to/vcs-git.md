...
Для добавления в систему проектов (Git репозиториев) на анализ необходимо предварительно создать подключение к системе контроля версий (VCS). CodeScoring поддерживает следующие системы контроля версий:

- GitLab
- GitHub
- BitBucket (только Data Center и Server)
- Azure DevOps Git
- Other Git (другие системы контроля версий, использующие Git)

Интеграция со всеми системами происходит через два возможных механизма: Personal Access Token или SSH. Для механизма Personal Access Token возможно подключение нескольких систем контроля версий с одним адресом, но разными токенами.
