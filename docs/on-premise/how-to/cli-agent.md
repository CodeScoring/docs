...
При отсутствии необходимого проекта для сохранения результатов в системе, проект можно добавить через параметр `--create-project` или указания в config-файле переменной:

- `create-project` — флаг создания нового проекта, по умолчанию стоит значение **false**.

**Важно**: результаты сканирования сохраняются только для CLI-проектов. 
