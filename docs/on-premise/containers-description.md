...
- **Migrate** – выполняет миграции для обновления структуры базы данных;
- **Osa-api** – обеспечивает взаимодействие между основной системой и плагином OSA (установленным в Sonatype Nexus Repository или JFrog Artifactory) через API;
...
- **Psql** – сервер PostgreSQL, используемый для управления базой данных;
...
- **Tasks-sca-external-scan** – очередь для сканирований, запущенных через консольный агент Johnny;
...
- **Judge** – отвечает за принятие решения о блокировке компонента или контейнерного образа (сверяется с настроенными политиками внутри платформы), во время работы сервиса OSA Proxy.