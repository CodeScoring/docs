...
Для распределения запросов между компонентами системы CodeScoring использует [Nginx](https://nginx.org/ru/docs/).
...
- **HUEY_SCA_EXTERNAL_SCAN_WORKERS** – количество воркеров отвечающих за очередь анализа, запущенного через агент Johnny `(tasks-sca-external-scan)`. Значение по умолчанию: `4`;
