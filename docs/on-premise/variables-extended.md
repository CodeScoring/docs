...
* **COMPOSE_PROJECT_NAME** - название проекта в docker compose, используется как префикс для ресурсов, создаваемых docker compose;

* **CODESCROING_VERSION** - обязательная переменная. Актуальную версию можно узнать в разделе [Changelog](/changelog/on-premise-changelog);
...
Для управления, обработки и хранениея данных CodeScoring использует `PostgreSQL`.
...
* **ANALYSIS_IGNORED_PATHS** - содержит указание путей, которые будут исключены из анализа. По умолчанию указаны следующие пути:
...
* **JOHNNY_BIN** - путь к исполняемому файлу агента сканирования `JOHNY`, значение по умолчанию: `/agents/johnny-linux-amd64`;
...
* **DJANGO_CACHES_REDIS_URL** - адрес сервера Redis, используемому в качестве системы кэширования Django, значение по умолчанию: `redis://redis:6379/1`;

* **ANALYSIS_ROOT** - корневая директория для клонирования репозиториев для анализа. Файлы копируются как временные и после анализа удаляются (используется только VCS- проектами), значение по умолчанию: `/analysis-root`;
...
* **HUEY_MEDIA_WORKERS** - количество воркеров Huey для отвечающих за загрузку медиа, значение по умолчанию: `4`;
...
* Переменные **REQUESTS_CA_BUNDLE** и **SSL_CERT_FILE** указывают на расположение SSL сертификатов используемых инсталляцией для работы.
  ...
  ```
* В случае, если Вам необходимо использовать Proxy для предоставления доступа инсталляции в интернет или к системе контроля версий. Необходимо расскоментировать и заполнить необходимые переменные указав URL адреса Proxy с протоколом, а так же имя пользователя и пароль при необходимости, например:
    * **HTTP_PROXY**
      ```bash
...
!!! warning "Переменные, связанные с Redis и Celery, определяют параметры конфигурации для взаимодействия с сервером Redis и настройки worker-ов Celery. Они влияют на производительность, надежность и устойчивость системы."

Переменные отвечающие за настройку соединения [Redis](/on-premise/containers-description) (Application Redis connection settings):

* **REDIS_BACKOFF_CAP** - максимальное количество повтора попыток соединения, при возникновении ошибок. Если после этого количества попыток соединение не установлено, то будет возвращена ошибка. Значение по умолчанию: `5`;
* **REDIS_BACKOFF_BASE** - базовое время задержки (в секундах) между попытками повтора при возникновении ошибок. Значение по умолчанию: `0.08`;
* **REDIS_RETRIES** - общее количество попыток подключения к серверу `Redis`, значение по умолчанию: `5`;
* **REDIS_SOCKET_CONNECT_TIMEOUT** - время ожидания (в секундах) при выполнении операций с сервером Redis. Если операция не завершится в течении этого времени, будет выдана ошибка. Значение по умолчанию: `5.0`;
...
Переменные отвечающие за настройку [Celery](/on-premise/containers-description):

* **CELERY_WORKER_CONCURRENCY** - количество задач, которые может обрабатывать один воркер Celery единовременно, значение по умолчанию: `6`;
...
* **USE_SMART_FILTERS** - отвечает за имользование "умных фильтров" в Celery. Эти фильтры позволяют более гибко управлять порядком выполняние задач. Доступны значения `True` или `False`, значение по умолчанию: `True`.

Следующие переменные отвечают за настройку архивирования данных модуля системы CodeScoring - OSA Proxy. По умолчанию переменные `OSA_ARCHIVE_THRESHOLD_DAYS, OSA_ARCHIVE_AUTO_CLEANUP_ENABLED, OSA_ARCHIVE_RETENTION_PERIOD_DAYS, OSA_ARCHIVE_CHUNK_SIZE` закомментированы, при необходимости их использования, необходимо их расскомментировать.
...
* **OSA_ARCHIVE_AUTO_CLEANUP_ENABLED** - включение или отключение автоматической очистки архивных компонентов OSA. Если `True`, то система будет автоматически удалять старые архивы, если `False` - очистка выполняется вручную, значение по умолчанию: `False`;
...
Следующие переменные отвечают за настройку компонента [Judge](/on-premise/containers-description). Их необходимо настраивать, только в случае использования OSA Proxy:

* **WEB_CONCURRENCY** - определяет количество воркереов для параллельной проверки компонентов или контейнеров на соответсвие политикам безопасности. Значение по умолчанию: `5`;
