...
!!! warning
    ...
    Данный вариант установки не предоставляет возможность горизонтального масштабирования CodeScoring. Для установки CodeScoring с поддержкой горизонтального масштабирования обратитесь к соответствующему разделу документации ниже.

!!! warning
    Необходимо наличие настроенного default `StorageClasss` в кластере. По умолчанию создаются тома **объемом 20 GiB**
...
!!! warning
    ...
    Описанные ниже опции являются **взаимоисключающими**. Необходимо выбрать **ТОЛЬКО ОДИН** вариант развертывания для каждого тома. Допускается выбор разных вариантов развертывания для разных томов.
...
!!! warning
    ...
    Для горизонтального масштабирования системы CodeScoring необходимо наличие в кластере Kubernetes взможности создания томов с типом доступа **ReadWriteMany (RWX)**
