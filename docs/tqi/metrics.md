...
## Графики влияние изменений

Расчет выполняется за период. Минимальный период - неделя.

Размер элемента на графике нормирован для выборки.

Нормирование выборки выполняется по формуле:
...
**Темп изменений** показывает объем изменения кода относительно общего объема строк кода. Высокий темп изменения кода может указывать на нестабильность и/или частые изменения требований.
...
**Скорость изменений** показывает объем изменений в коде относительно количества коммитов. Высокая скорость может приводить к потере качества.
...
**Плотность изменений** показывает какой объем изменений в коде был произведен относительно количества измененных файлов. Большое значение плотности может свидетельствовать об модификации большого количества модулей и необходимо внимательного разностороннего тестирования.
