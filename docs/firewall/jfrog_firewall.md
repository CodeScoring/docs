...
- **skipScan** – опция, позволяющая пропустить сканирование компонентов для текущего пользователя;
...
- **blockDownloads** - опция блокирования загрузки компонентов. В случае выставления значения `false` компоненты будут загружаться в независимости от наличия ошибок CodeScoring API или плагина;
- **updateArtifactProperties** – опция обновления информации о компоненте, используя результаты анализа с инсталляции CodeScoring. Необходимо выставить значение `false` при наличии ошибки "Could not acquire lock".
- **workMode** – режим работы плагина.
