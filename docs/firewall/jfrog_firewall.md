...
- **indexUrl** - ссылка на страницу компонента в индексе.

При наличии признака `blockDownloads=true` загрузка компонентов, не прошедших проверку, блокируется на этапе попадания в прокси-репозиторий. Ответ от плагина в таком случае имеет следующее содержание:
...
```
{
    "errors": [
        { 
            "status": 403,
            "message": "Download request has
            been canceled: CVSS3 score
            2High, Vuln block"
        }
    ]
}
